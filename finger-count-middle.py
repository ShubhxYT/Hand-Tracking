...
while True:
    ...
    if len(lmList) != 0:
        ...
        # checking for 0 fingers specifically
        if fingers.count(0) == 5:
            h, w, c = overlayList[5].shape  # Assuming 6.png is the 6th image in overlayList
            img[0:h, 0:w] = overlayList[5]
        
        # Check for middle finger specifically
        if totalFingers == 1 : 
            if fingers[2] == 1: #checking if the middle finger is up
                h, w, c = overlayList[6].shape  # Assuming 7.png is the 7th image in overlayList
